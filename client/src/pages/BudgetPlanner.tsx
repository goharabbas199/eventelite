    ...
    <Layout title="Budget Planner">
      ...
      {selectedClient && selectedClientId ? (
        ...
        <div className="text-center py-20 text-slate-400">
          Please select a client to view budget.
        </div>
      )}
    </Layout>
  );
...
function ClientBudgetView({
  ...
  const servicesTotal = plannedServices.reduce(
    (sum, s) => sum + Number(s.cost),
    0,
  );

  const venueCost = client.venueId
    ? Number(
        // find venue base price from client object if available later
        client.venue?.basePrice || 0,
      )
    : 0;
      ...
      <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="border-none shadow-md bg-white">
          <CardContent className="p-6">
            ...
            <h3
              className={`text-3xl font-bold mt-2 ${remaining < 0 ? "text-red-500" : "text-emerald-500"}`}
            >
...
function PaidToggle({ expense, clientId }: { expense: any; clientId: number }) {
  ...
  return (
    <div
      ...
      onClick={() =>
        mutate({ id: expense.id, clientId, isPaid: !expense.isPaid })
      }
    >
      {expense.isPaid ? (
        <>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Buffet deposit" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
