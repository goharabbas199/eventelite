      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredVenues.map((venue) => (
            <Card
              key={venue.id}
              className="group relative border border-border/60 shadow-sm hover:shadow-lg hover:border-indigo-200 hover:-translate-y-1 transition-all duration-200 rounded-2xl overflow-hidden bg-white"
            >
              {/* Action menu */}
              <div className="absolute top-3 right-3 z-20" onClick={(e) => e.preventDefault()}>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="bg-white/90 backdrop-blur-sm border border-border/60 rounded-xl p-1.5 hover:bg-slate-50 transition shadow-sm">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-border/60 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredVenues.map((venue) => (
                <tr key={venue.id} className="border-t border-border/40 hover:bg-slate-50/80 transition-colors">
