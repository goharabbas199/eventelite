...
import {
  ...
  CheckCircle, ArrowLeft, RefreshCw, ShieldCheck, Loader2,
} from "lucide-react";
    ...
    <div className="flex gap-2.5 justify-center">
      {[0, 1, 2, 3, 4, 5].map((i) => (
        <input
          ...
          disabled={disabled}
          style={{ height: "52px" }}
          className="w-11 text-center text-xl font-bold border-2 rounded-xl
            ...
            <div className="text-center mt-5">
              <p className="text-xs text-slate-400">
                ...
                {resendCooldown > 0 ? (
                  ...
                  <button type="button" onClick={() => { setError(""); resendMutation.mutate(); }}
            ...
            <form onSubmit={handleReset} className="space-y-4 mt-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input type={showNewPw ? "text" : "password"} placeholder="••••••••" value={newPassword} onChange={(e) => setNewPassword(e.target.value)}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input type="password" placeholder="••••••••" value={confirmNewPassword} onChange={(e) => setConfirmNewPassword(e.target.value)}
                    className="pl-10 h-11 rounded-xl bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700"
                    autoComplete="new-password" data-testid="input-confirm-new-password" />
                </div>
            ...
            <div className="text-center mt-5">
              <p className="text-xs text-slate-400">
                ...
                {resendCooldown > 0 ? (
                  ...
                  <button type="button" onClick={() => { setError(""); forgotMutation.mutate(); }}
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12">
        ...
        <div className="w-full max-w-sm">
          ...
          <button
            ...
            className="flex items-center justify-center gap-3 w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition-all duration-200 shadow-sm mb-4 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 rounded-xl p-1 mb-5 gap-1">
            <button type="button" onClick={() => switchToMode("signin")}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${
                mode === "signin"
                  ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm"
                  : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
              }`} data-testid="tab-signin">Sign In</button>
            <button type="button" onClick={() => switchToMode("signup")}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${
                mode === "signup"
                  ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm"
                  : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
              }`} data-testid="tab-signup">Sign Up</button>
          </div>
            ...
            <form onSubmit={handleSignin} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button type="button" onClick={() => { setEmail(email); switchToMode("forgot-password"); }}
                    className="text-xs text-indigo-500 hover:text-indigo-700 dark:hover:text-indigo-300 font-semibold transition-colors"
                    data-testid="button-forgot-password">Forgot password?</button>
                </div>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input type={showPw ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)}
                    className="pl-10 pr-10 h-11 rounded-xl bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700"
                    autoComplete="new-password" data-testid="input-password" />
                  <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                    {showPw ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
