...
export default function Clients() {
  ...
  const nativeSelectCls =
    "h-9 border-0 border-l border-slate-100 dark:border-slate-700 px-3 text-sm " +
    "bg-transparent text-slate-600 dark:text-slate-300 focus:outline-none focus:ring-0 " +
  ...
  return (
    <Layout title="Clients">
      ...
      {/* Combined filter bar */}
      <div className="bg-white dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden">
        <div className="flex items-stretch divide-x divide-slate-100 dark:divide-slate-700">
          ...
          <div className="flex-1 flex items-center gap-2 px-3">
            ...
            <input
              ...
              className="flex-1 h-10 bg-transparent text-sm text-slate-700 dark:text-slate-300 placeholder-slate-400 focus:outline-none min-w-0"
            />
            {search && (
        ...
        {/* Filter meta row */}
        <div className="flex items-center justify-between px-4 py-2 border-t border-slate-100 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-700/20">
          <p className="text-xs text-slate-400">
            Showing <span className="font-semibold text-slate-600 dark:text-slate-300">{filteredCount}</span> of {totalClients} clients
          </p>
          {hasActiveFilters && (
            <button
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-slate-100 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-700/30">
                <TableHead className="pl-5 text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  Name
                </TableHead>
                <TableHead
                  className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 cursor-pointer hover:text-indigo-600 select-none"
                ...
                <TableHead
                  className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 cursor-pointer hover:text-indigo-600 text-right select-none"
                ...
                <TableHead className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 pr-5">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {isLoading ? (
                ...
                filteredClients.map((client) => {
                  ...
                  return (
                    ...
                    >
                      <TableCell className="font-semibold text-sm text-slate-800 dark:text-slate-200 pl-5 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                        {client.name}
                      </TableCell>
                      ...
                      <TableCell className="text-right font-bold text-sm text-slate-800 dark:text-slate-200">
                        {client.budget ? `$${Number(client.budget).toLocaleString()}` : "—"}
                      </TableCell>
                      <TableCell>
                        <span className={`chip ${STATUS_STYLE[client.status] ?? "badge-slate"}`}>
